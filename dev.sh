...
if ! podman pod inspect smsbar-images-dev_pod >/dev/null
then
  podman pod create --name smsbar-images-dev_pod
fi

# See https://github.com/xd009642/tarpaulin/issues/1087 for the seccomp thing
podman run --detach --name smsbar-images-dev --pod smsbar-images-dev_pod --security-opt seccomp=~/src/lunarvim_rust/seccomp.json -w /root/src/smsbar-images \
...
podman exec -it smsbar-images-dev bash
